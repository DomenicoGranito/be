...
class Playlist: UITableViewController, UISearchResultsUpdating, PlaylistDelegate
{
    ...
    {
        let request:NSFetchRequest<NSFetchRequestResult>=NSFetchRequest(entityName:"Playlist")
        ...
        if(playlists?.count >= 1)
        {
            ...
            return songsUnsorted.sortedArray(using: [songSortDescriptor]) as [Any] as NSArray
        }
        return []
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if resultSearchController.isActive && resultSearchController.searchBar.text != "" {
            songName = (filteredSongs[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "title") as! String
            duration = (filteredSongs[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "durationStr") as! String
            identifier = (filteredSongs[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "identifier") as! String
            imageData = (filteredSongs[(indexPath as NSIndexPath).row] as AnyObject).value(forKey: "thumbnail") as! Data
        }
            
        else{
            let row = x[(indexPath as NSIndexPath).row]
        ...
        if let selectedIndexPaths = tableView.indexPathsForSelectedRows as [IndexPath]?
        {
            ...
            if resultSearchController.isActive && resultSearchController.searchBar.text != "" {
                ...
                for indexPath : IndexPath in selectedIndexPaths {
                    ...
                    var selectedRow = (indexPath as NSIndexPath).row
                ...
                for indexPath : IndexPath in selectedIndexPaths {
                    selectedRows += [(indexPath as NSIndexPath).row]
                    let row = x[(indexPath as NSIndexPath).row]
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == UITableViewCellEditingStyle.delete {
            ...
            if resultSearchController.isActive && resultSearchController.searchBar.text != ""{
                ...
                row = x[(indexPath as NSIndexPath).row]
                x.remove(at: (indexPath as NSIndexPath).row)
            }
    ...
    func setupPlayerQueue(){
        ...
        if resultSearchController.isActive && resultSearchController.searchBar.text != ""{
            ...
            curNdx = ((tableView.indexPathForSelectedRow as NSIndexPath?)?.row)!
        }
